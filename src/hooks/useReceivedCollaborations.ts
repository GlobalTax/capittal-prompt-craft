...
export const useReceivedCollaborations = () => {
  const [requests, setRequests] = useState<ReceivedCollaborationRequest[]>([]);
  ...
  const fetchRequests = async () => {
    try {
      ...
      const advisorIds = [...new Set(data?.map(r => r.requesting_advisor_id) || [])];
      const { data: profiles } = await supabase
        .from('user_profiles')
        .select('id, first_name, last_name, company')
        .in('id', advisorIds);

      // Map requests with advisor data
      const mappedRequests = (data || []).map(request => ({
        ...request,
        requesting_advisor: profiles?.find((p: any) => p.id === request.requesting_advisor_id),
      })) as ReceivedCollaborationRequest[];
